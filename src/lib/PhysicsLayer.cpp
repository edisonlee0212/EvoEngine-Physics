...
YAML::Emitter &EvoEngine::operator<<(YAML::Emitter &out, const PxVec3 &v)
{
    out << YAML::Flow;
...
YAML::Emitter &EvoEngine::operator<<(YAML::Emitter &out, const PxVec4 &v)
{
    out << YAML::Flow;
...
YAML::Emitter &EvoEngine::operator<<(YAML::Emitter &out, const PxMat44 &v)
{
    out << YAML::Flow;
...
void PhysicsLayer::PreUpdate()
{
    ...
    const auto activeScene = GetScene();
...
void PhysicsLayer::OnCreate()
{
    ...
    m_defaultPhysicsMaterial =
        Resources::CreateResource<PhysicsMaterial>("Default");
#pragma endregion
}
...
void PhysicsSystem::Simulate(float time) const
{
	...
	TransformGraph::GetInstance().m_physicsSystemOverride = true;
}
